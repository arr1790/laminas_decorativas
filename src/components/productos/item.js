            ...
            <div className="flex-grow flex items-center justify-center overflow-hidden rounded-lg relative">
              ...
              >
                <img
                  ...
                  alt={producto.name}
                  className={`w-full h-full max-h-[700px] object-contain object-center rounded-md shadow transition-all duration-300 ${zoomActive ? 'cursor-crosshair' : 'cursor-default'
                    }`}
                  style={{
                ...
                <button
                  onClick={toggleZoom}
                  className={`absolute bottom-4 right-4 p-2 rounded-full transition-all ${zoomActive ? 'bg-indigo-600 text-white' : 'bg-white/80 text-gray-800 hover:bg-white'
                    } shadow-md`}
              ...
              <div className="flex mt-4 space-x-2 overflow-x-auto py-2">
                {imagenes.map((img, index) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 ${imagenActual === index ? 'border-indigo-600' : 'border-gray-200'
                      }`}
                  >
                    <img
                      src={img}
