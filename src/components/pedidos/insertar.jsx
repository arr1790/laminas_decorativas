  ...
  return (
    <form id={formId} action={action} className="flex flex-col gap-4">
      ...
      >
        {pending ? (
          <div>
            <RefreshCwIcon className="inline animate-spin" /> Guardando...
          </div>
        ) : (
          <div>
            <PlusIcon className="inline" /> Guardar
          </div>
        )}
      </button>

      {user?.id && <input type="hidden" name="userId" defaultValue={user.id} />}

      <p className="font-bold">Seleccionar productos:</p>
        ...
        {products.map((product) => (
          ...
          >
            ...
            <input
              ...
              min={1}
              className="w-full border mt-2 p-1 text-center rounded"
            />
          </CheckBox>
        ))}
      </div>
